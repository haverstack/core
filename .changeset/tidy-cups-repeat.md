...
Two consequences follow: the owner may name a `relationship` target that does not exist, exactly as an unscoped `Stack` allows, and `restoreVersion()` now honors the spec's promise that a snapshot naming a since-hard-deleted container restores anyway for the owner.
