...
describe('GET /records round trip', () => {
  const cases: Array<[string, StackQuery]> = [
    ...
    ['attachment label alone', { filter: { attachment: { label: 'cover' } } }],
    ['attachment fileId alone', { filter: { attachment: { fileId: 'file-abc123' } } }],
    [
      'attachment label and fileId',
      { filter: { attachment: { label: 'cover', fileId: 'file-abc123' } } },
    ],
    ['referencesFileId', { filter: { referencesFileId: 'file-abc123' } }],
    ...
    [
      'every native field at once',
      {
        filter: {
          ...
          tags: ['starred'],
          attachment: { label: 'cover', fileId: 'file-abc123' },
          referencesFileId: 'file-abc123',
