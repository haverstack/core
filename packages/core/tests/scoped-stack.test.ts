...
describe('ScopedStack — write access', () => {
  test('owner can update any record', async () => {
    const record = await adapter.createRecord(makeRecord());
    const updated = await stack.asEntity(OWNER).patchContent(record.id, { text: 'hi' });
  ...
  test('a permissions change set rejects a write-access holder that is not creator or stack owner', async () => {
    ...
    await stack.asEntity(OWNER).mutate(record.id, { permissions: perms });
  ...
  test('write methods throw StackNotFoundError (not StackPermissionError) for a missing record', async () => {
    await expect(stack.asEntity(OWNER).patchContent('nonexistent', {})).rejects.toThrow(
      StackNotFoundError,
    );
    await expect(stack.asEntity(OWNER).patchContent('nonexistent', {})).rejects.not.toThrow(
      StackPermissionError,
    );
  });
});
...
describe('ScopedStack.mutate — the `unlisted` key', () => {
  test('rejects write-access holder that is not creator or stack owner', async () => {
    ...
    await stack.asEntity(OWNER).mutate(record.id, { unlisted: true });
  ...
  test('a stranger with no access gets StackNotFoundError', async () => {
    const record = await adapter.createRecord(makeRecord());
    await expect(stack.asEntity(STRANGER).mutate(record.id, { unlisted: true })).rejects.toThrow(
      StackNotFoundError,
    );
  });

  test('the record creator (not stack owner) may toggle it', async () => {
    const record = await adapter.createRecord(makeRecord({ createdBy: { subjectId: MEMBER } }));
    await stack.asEntity(MEMBER).mutate(record.id, { unlisted: true });
    expect((await adapter.getRecord(record.id))?.unlistedAt).toBeInstanceOf(Date);
  });
});
...
describe('ScopedStack — record-existence disclosure', () => {
  ...
  test('a create-only grantee cannot confirm the records it wrote beside', async () => {
    ...
    for (const id of [own.id, sibling.id, 'nonexistent']) {
      ...
      await expect(view.patchContent(id, { text: 'edited' })).rejects.toThrow(StackNotFoundError);
    }
  });
  ...
  test('a _grant record discloses neither its existence nor its family to a non-owner', async () => {
    ...
    const [grantRecord] = (await stack.listTypeGrants({
      kind: 'entity',
      entityId: MEMBER,
    })) as StackRecord[];
    const view = stack.asEntity(MEMBER);
    ...
    await expect(view.patchContent(grantRecord.id, { actions: ['read-any'] })).rejects.toThrow(
      StackNotFoundError,
    );
    await expect(view.mutate(grantRecord.id, { permissions: perms })).rejects.toThrow(
      StackNotFoundError,
    );
  });
});
...
describe('ScopedStack — versions', () => {
  ...
  // patchContent()/associate()'s gate exactly.
  ...
  test('getVersions/getVersion serve the same rows to a write-holder and to the owner', async () => {
    ...
    await adapter.saveVersion(record.id, {
      ...
      createdBy: { subjectId: 'someone-else' },
    });
    ...
    expect(memberList[0].createdBy?.subjectId).toBe('someone-else');
  });
  ...
  describe('restoreVersion — reference-reconveyance gating', () => {
    ...
    test('an owner principal acting for someone else is not exempt', async () => {
      const record = await photoRecordWithUnreachableSnapshot();
      await expect(
        stack.asActor({ principalId: OWNER, subjectId: MEMBER }).restoreVersion(record.id, 1),
      ).rejects.toThrow(StackPermissionError);
    });
...
describe('ScopedStack.query', () => {
  ...
  describe('includeUnlisted', () => {
    ...
    test('an owner-delegated app does not inherit it', async () => {
      const scoped = stack.asActor({ principalId: 'app-did', subjectId: OWNER });
...
describe('ScopedStack.create', () => {
  ...
  // A scoped write always names its author, so an absent createdBy means
  // exactly one thing: an unscoped Stack wrote the record.
  test('owner writing through asEntity(ownerEntityId) stamps createdBy', async () => {
    const record = await stack.asEntity(OWNER).create(COMMENT, { text: 'hello' });
    expect(record.createdBy?.subjectId).toBe(OWNER);
  ...
  // owner-authored record carrying no createdBy can never accidentally
  ...
  // The authenticated tier stops at the people who turn up with a DID. A
...
describe('ScopedStack — group-targeted grants', () => {
  ...
  test('a group grantee with an empty groupId denies rather than reaching anyone', async () => {
    // grantType() refuses to write this and the schema refuses a grantee that
    // names no tier; a _grant Record reaching storage another way is still
    // an ordinary Record, so evaluation has to refuse it again.
    await adapter.createRecord(
      makeRecord({
        typeId: '_grant@1',
        content: {
  ...
  test('an entity grantee with an empty entityId denies rather than reaching anyone', async () => {
    await adapter.createRecord(
      makeRecord({
        typeId: '_grant@1',
        content: {
  ...
  // The tier a Grant reaches is read off its `kind`, so a Grant that lost
  // its grantee — or carries one the vocabulary does not name — reaches
  // nobody. There is no shape absence can take that widens reach.
  test('a grant carrying no grantee reaches nobody', async () => {
    await adapter.createRecord(
      makeRecord({
        typeId: '_grant@1',
  ...
  test('a grant naming an unknown grantee kind reaches nobody', async () => {
    await adapter.createRecord(
      makeRecord({
        typeId: '_grant@1',
        content: {
  ...
  // The narrower roster tier: `role: 'admin'` asks for an admin, and a
...
describe('ScopedStack — grant-based update/delete', () => {
  ...
  test('update-own: entity can update a record they authored', async () => {
    ...
    const updated = await stack.asEntity(MEMBER).patchContent(record.id, { text: 'edited' });
  ...
  test('an authenticated-tier grant applies update-own to any authenticated entity', async () => {
    ...
    const updated = await stack.asEntity(STRANGER).patchContent(record.id, { text: 'edited' });
    expect(updated.content.text).toBe('edited');
  });
});
...
describe('ScopedStack — write implies read', () => {
  ...
  test('read-own does not license update-any, and leaves the read it does convey intact', async () => {
    ...
    const view = stack.asEntity(MEMBER);
    await expect(view.patchContent(own.id, { text: 'edited' })).rejects.toThrow(
      StackPermissionError,
    );
    await expect(view.patchContent(theirs.id, { text: 'edited' })).rejects.toThrow(
      StackNotFoundError,
  ...
  test('a delegated principal needs the read companion on its own side', async () => {
    ...
    await expect(
      stack
        .asActor({ principalId: APP, subjectId: MEMBER })
        .patchContent(record.id, { text: 'edited' }),
    ).rejects.toThrow(StackNotFoundError);
  });
});
...
describe('ScopedStack — blind write', () => {
  test('a create-only grantee writes records it cannot read back or enumerate', async () => {
    ...
    expect(posted.createdBy?.subjectId).toBe(MEMBER);
...
describe('ScopedStack.commitMigration', () => {
  ...
  // the same rule deleteAttachment() and a reshare apply.
...
describe('ScopedStack.putAttachment', () => {
  ...
  test('upload creates _attachment@1 record owned by the uploader', async () => {
    ...
    expect(result.records).toHaveLength(1);
    expect(result.records[0].createdBy?.subjectId).toBe(MEMBER);
  ...
  test('returns the attributed record, so the uploader needs no follow-up query', async () => {
    ...
    expect(await stack.get(record.id)).toMatchObject({
      id: record.id,
      createdBy: { subjectId: MEMBER },
    });
  });
...
describe('ScopedStack — group role gating', () => {
  ...
  test('plain member cannot update group content', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(group.id, { name: 'renamed' }),
    ).rejects.toThrow(StackNotFoundError);
  });
  ...
  test('stack owner can manage the group regardless of roster membership', async () => {
    const group = await makeGroup();
    const updated = await stack.asEntity(OWNER).patchContent(group.id, { name: 'renamed' });
    expect(updated.content.name).toBe('renamed');
  ...
  test('a permissions change set on a group requires admin, not just record authorship', async () => {
    const group = await makeGroup({ createdBy: { subjectId: MEMBER } });
    const perms: AuthorityAssociation[] = [{ kind: 'anyone', label: 'read' }];
    ...
    await stack.asEntity(ADMIN).mutate(group.id, { permissions: perms });
  ...
  test('an unlisted change set on a group requires admin, not just record authorship', async () => {
    ...
    await stack.asEntity(ADMIN).mutate(group.id, { unlisted: true });
  ...
  test('owner writing through ScopedStack stamps the owner as admin exactly once', async () => {
    ...
    expect(group.createdBy?.subjectId).toBe(OWNER);
    const adminAssociations = (group.associations ?? []).filter(
      (a) =>
        a.kind === 'relationship' &&
        a.label === 'admin' &&
...
describe('ScopedStack — the write bit reaches no part of the ACL', () => {
  ...
  test('revokeAccess() is refused to a write-holder and allowed to the creator', async () => {
    const record = await adapter.createRecord(
      makeRecord({
        createdBy: { subjectId: MEMBER },
        permissions: [readFor(MEMBER), readFor(STRANGER)],
      }),
    );
    await expect(
...
describe('_group — a deleted Group carries no roster', () => {
  ...
  // listTypeGrants() shares grantCoversGrantee() with the access checks, so a
  ...
  test('coverage listing drops a grant reaching through a deleted Group', async () => {
    ...
    expect(await stack.listTypeGrants({ kind: 'entity', entityId: MEMBER })).toHaveLength(1);

    await stack.delete(group.id);
    expect(await stack.listTypeGrants({ kind: 'entity', entityId: MEMBER })).toHaveLength(0);
  });
...
// -------------------------------------------------------
//
// grantType() refuses these at the write; a Record arriving from an import, a
...
describe('_grant — a malformed typeId or actions confers nothing', () => {
  ...
  test('revokeType() reports no match rather than throwing, and listTypeGrants still shows it', async () => {
    ...
    // revokeType() matches the stored list at grantType()'s own granularity, and a
...
describe('ScopedStack.mutate — the `parentId` key', () => {
  ...
  test('read on the destination is required', async () => {
    await expect(
      stack.asEntity(MEMBER).mutate(writable.id, { parentId: unreadableBox.id }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('a delegated principal does not inherit the owner exemption', async () => {
    await expect(
      stack
        .asActor({ principalId: OWNER, subjectId: MEMBER })
        .mutate(writable.id, { parentId: MISSING_ID }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('write on the record is required', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).mutate(readOnly.id, { parentId: readableBox.id }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('a soft-deleted record is refused', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).mutate(writable.id, { parentId: readableBox.id }),
    ).rejects.toThrow(StackConflictError);
  });
  ...
  test('an unreadable record is not found, never forbidden', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).mutate(hidden.id, { parentId: readableBox.id }),
    ).rejects.toThrow(StackNotFoundError);
  });

  test('the owner is exempt from the destination gate', async () => {
    const moved = await stack.asEntity(OWNER).mutate(writable.id, { parentId: unreadableBox.id });
    expect(moved.parentId).toBe(unreadableBox.id);
  });

  // A move bumps nothing, so it leaves the record's updatedBy
  ...
  test('the actor travels on the journal entry rather than the record', async () => {
    ...
    expect(entries.at(-1)?.actor).toEqual({ subjectId: MEMBER });
  });
});
...
describe('ScopedStack — file-ref content field gating', () => {
  ...
  test('patchContent() rejects changing a file-ref field to an inaccessible file', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(record.id, { coverFileId: FILE_ID }),
    ).rejects.toThrow(StackPermissionError);
  });

  test('patchContent() leaving the file-ref field untouched is unaffected by its accessibility', async () => {
    ...
    const updated = await stack.asEntity(MEMBER).patchContent(record.id, { title: 'renamed' });
...
describe('ScopedStack — delegation', () => {
  ...
  test('asActor() scopes to the principal and subject a token names', async () => {
  ...
  test('asActor() with one identity is the undelegated case', async () => {
    await grantAll({ kind: 'entity', entityId: MEMBER });
    const record = await stack
      .asActor({ principalId: MEMBER, subjectId: MEMBER })
      .create(COMMENT, { text: 'hi' });
    expect(record.createdBy?.subjectId).toBe(MEMBER);
    expect(record.createdBy?.principalId).toBeUndefined();
  });
  ...
  test('a swapped pair attributes the write to the app instead of the person', async () => {
    ...
    const swapped = await stack
      .asActor({ principalId: MEMBER, subjectId: APP })
      .create(COMMENT, { text: 'hi' });
    expect(correct.createdBy?.subjectId).toBe(MEMBER);
    expect(swapped.createdBy?.subjectId).toBe(APP);
  });
  ...
  test('a record shared by permission stays unreachable through an app with no grant on its type', async () => {
    ...
    expect(await stack.asActor({ principalId: APP, subjectId: MEMBER }).get(shared.id)).toBeNull();
  });
  ...
  test('an app delegated for the owner cannot set permissions on its subject records', async () => {
    await grantAll({ kind: 'entity', entityId: APP });
    await grantAll({ kind: 'authenticated' });
    const view = stack.asActor({ principalId: APP, subjectId: OWNER });
  ...
  test('an owner principal cannot set a card binding for a subject holding write on it', async () => {
    ...
    await expect(view.patchContent(card.id, { appId: 'com.example.bank' })).rejects.toThrow(
      StackPermissionError,
    );
  ...
  test('a delegated app may still create records it does not try to share', async () => {
    ...
    await grantAll({ kind: 'authenticated' });
    const record = await stack
      .asActor({ principalId: APP, subjectId: OWNER })
  ...
  test('an app delegated for a group admin cannot manage the group', async () => {
    ...
    await expect(
      stack
        .asActor({ principalId: APP, subjectId: MEMBER })
        .patchContent(group.id, { name: 'Hijacked' }),
    ).rejects.toThrow(StackNotFoundError);
  });
  ...
  test('a default grant does not satisfy the principal side of the intersection', async () => {
    ...
    expect(direct.createdBy?.subjectId).toBe(MEMBER);
  });
  ...
  test('an owner principal cannot manage a group its subject does not administer', async () => {
    ...
    expect(
      await stack
        .asActor({ principalId: OWNER, subjectId: MEMBER })
        .patchContent(group.id, { name: 'Renamed' }),
    ).toBeTruthy();
  });
  ...
  test('records are queryable by the principal that wrote them', async () => {
    ...
    await stack
      .asActor({ principalId: APP, subjectId: OWNER })
      .create(COMMENT, { text: 'via app' });
    await stack.asEntity(OWNER).create(COMMENT, { text: 'direct' });

    const viaApp = await stack.query({ filter: { createdBy: { principalId: APP } } });
...
describe('ScopedStack — _app bindings', () => {
  ...
  test.each([
    ...
    for (const field of ['did', 'appId']) {
      ...
      try {
        await stack
          .asEntity(actor)
          .patchContent(shared.id, { name: 'Renamed', [field]: undefined });
      } catch (e) {
  ...
  test('a write-holder may round-trip unchanged bindings in a full-content update', async () => {
    ...
    const updated = await scoped.patchContent(shared.id, { ...current!.content, name: 'Renamed' });
...
describe('ScopedStack — _app.appId is owner-only', () => {
  ...
  test('a non-owner with write on a card cannot change its appId', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(shared.id, { appId: 'com.example.bank' }),
    ).rejects.toThrow(StackPermissionError);
  });
...
describe('ScopedStack — _entity bindings hold under a grant', () => {
  ...
  test('a grantee may write a petname card but not repoint an existing one', async () => {
    ...
    await expect(scoped.patchContent(alice.id, { name: 'Alice Smith' })).resolves.toBeDefined();

    // Repointing is the impersonation and is refused.
    await expect(scoped.patchContent(alice.id, { did: MEMBER })).rejects.toThrow(
      StackValidationError,
    );
  });
  ...
  test('a grantee may relabel the owner card while round-tripping its DID', async () => {
    ...
    const updated = await scoped.patchContent(ownerCard.id, {
      ...current!.content,
      name: 'The Owner',
    });
...
describe('ScopedStack — _grant records are owner-write-only', () => {
  ...
  test('a write-holder cannot widen the actions on their own grant', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(grantRecord.id, { actions: ['read-any'] }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('a write-holder cannot retarget a grant at another type', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(grantRecord.id, { typeId: COMMENT }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('a write-holder cannot reassign a grant to another grantee', async () => {
    ...
    await expect(
      stack.asEntity(MEMBER).patchContent(grantRecord.id, {
        grantee: { kind: 'entity', entityId: STRANGER },
      }),
    ).rejects.toThrow(StackPermissionError);
  });
  ...
  test('an owner principal acting for someone else cannot write a grant record', async () => {
    ...
    await expect(
      stack.asActor({ principalId: OWNER, subjectId: MEMBER }).patchContent(grantRecord.id, {
        actions: ['read-any'],
      }),
    ).rejects.toThrow(StackPermissionError);
  });
...
describe('ScopedStack — soft delete presents a tombstone', () => {
  ...
  test('get() withholds content, associations and authorship', async () => {
    ...
    expect(got!.createdBy?.subjectId).toBeUndefined();
...
describe('ScopedStack — mutating a soft-deleted record', () => {
  ...
  test('patchContent() is refused — undelete first', async () => {
  ...
  // a reshare and the `unlisted` key reach their own gate rather than
  ...
  test('a reshare and the `unlisted` key are refused for a resharer too', async () => {
    const record = await deleted();
    const owner = stack.asEntity(OWNER);
    await expect(owner.mutate(record.id, { permissions: [] })).rejects.toThrow(StackConflictError);
    await expect(owner.mutate(record.id, { unlisted: true })).rejects.toThrow(StackConflictError);
  });
  ...
  test('a stranger gets not-found, not the deleted-state conflict', async () => {
    const record = await stack.create(NOTE, { text: 'private' });
    await stack.delete(record.id);
    await expect(stack.asEntity(STRANGER).patchContent(record.id, { text: 'x' })).rejects.toThrow(
      StackNotFoundError,
    );
  });
...
describe('ScopedStack.mutate — authority resolves per key', () => {
  let note: StackRecord;

  beforeEach(async () => {
    // MEMBER holds write but is not the author, so it may edit and may not
    // reshare — the split the whole per-key rule exists for.
    note = await stack.create(
      NOTE,
      { text: 'hello' },
      {
        createdBy: { subjectId: OWNER },
  ...
  test('a write-holder may edit content', async () => {
    const scoped = stack.asEntity(MEMBER);
    const edited = await scoped.mutate(note.id, { contentPatch: { text: 'edited' } });
    expect(edited.content).toEqual({ text: 'edited' });
  });
  ...
  // The refusal is the whole call: the content patch is one the same caller
  // could have made on its own, and it must still not land.
  test('a refused reshare key refuses the edit beside it', async () => {
    const scoped = stack.asEntity(MEMBER);
    await expect(
  ...
  test('the owner may edit and reshare in one call', async () => {
    const scoped = stack.asEntity(OWNER);
  ...
  // change set would leave it. Widening access in the same call must not
  // satisfy the read check the destination owes.
  test('a widened permissions key cannot satisfy the parentId read check', async () => {
    const secret = await stack.create(
      NOTE,
      { text: 'secret' },
      { createdBy: { subjectId: OWNER } },
    );
  ...
  test('a readable destination is reachable by a write-holder', async () => {
    const readable = await stack.create(
      NOTE,
      { text: 'box2' },
      {
        createdBy: { subjectId: OWNER },
    ...
    const scoped = stack.asEntity(MEMBER);
    const moved = await scoped.mutate(note.id, { parentId: readable.id });
    expect(moved.parentId).toBe(readable.id);
  });

  test('an anonymous requester reaches nothing, whatever the change set names', async () => {
    const anon = stack.asEntity(null);
    await expect(anon.mutate(note.id, { contentPatch: { text: 'x' } })).rejects.toThrow(
      StackNotFoundError,
    );
  });

  test('a malformed change set is a validation error before any gate', async () => {
  ...
  test('patchContent carries the same gate as a content-only change set', async () => {
    const stranger = stack.asEntity(STRANGER);
    await expect(stranger.patchContent(note.id, { text: 'x' })).rejects.toThrow(StackNotFoundError);
  });
});

describe('ScopedStack.mutate — delegated principals', () => {
  let note: StackRecord;

  beforeEach(async () => {
    note = await stack.create(NOTE, { text: 'hello' }, { createdBy: { subjectId: OWNER } });
  });

  // Widening access is what containment most needs to hold, so an app with
  // no standing of its own is refused it whichever key reaches for it —
  // even acting for the owner, who holds it.
  test('a delegated app cannot reshare while acting for the owner', async () => {
    const app = stack.asActor({ principalId: 'did:key:zApp', subjectId: OWNER });
  ...
  // The rule is intersected, so it also runs the other way: an owner
  // principal never carries its subject to a reshare the subject lacks.
  test('an owner principal cannot reshare on behalf of a subject who could not', async () => {
    ...
    const forMember = stack.asActor({ principalId: OWNER, subjectId: MEMBER });
    await expect(
