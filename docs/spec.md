...
Returns `413 Request Entity Too Large` (code `payload_too_large`, reconstructed client-side as `StackPayloadTooLargeError` — see [Error responses](#error-responses)) if the payload exceeds the server's configured limit (default 50 MB, controlled by `MAX_ATTACHMENT_BYTES`). The same limit is exposed ahead of time as `maxAttachmentBytes` in [discovery](#discovery); `Stack.putAttachment()`/`ScopedStack.putAttachment()` (#114 C4) pre-check it client-side and throw `StackPayloadTooLargeError` before sending, so apps normally see the failure without burning the upload — this endpoint's 413 is the authoritative backstop, not the only enforcement point.
