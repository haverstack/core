...
| **409** | Conflict                 | `StackConflictError` — operation blocked by a constraint violation (e.g. deleting an attachment still referenced by a record, a client-supplied `id` that already exists, or a version-number collision between two racing writers — see [Versions](#versions))                                                                                                                                                                                                                               |
