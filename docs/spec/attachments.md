...
**The pointer annotates a reference; it does not name one.** Association identity stays `(kind, label)` plus `fileId` ([Data model § Associations](./data-model.md#associations)), so `dissociate()` matches without it, and an `associate()` naming a different `attachmentRecordId` for an association the record already holds **re-points that association in place** rather than adding a second reference to the same file. Re-pointing is still a write: it reports an `associate` op on the change feed, carrying the association's new `attachmentRecordId` in that frame's `associationsAdded` — though — like every `associate()` — it never bumps `version` (see [Versioning § Version history](./versioning.md#version-history)). The old `attachmentRecordId` is not reported on the feed: a frame carries what is current, so a subscriber watching at that moment sees only the new value (see [Events § The event shape](./events.md#the-event-shape)). What the re-point replaced is kept by [the change journal](./journal.md), on that entry's [`repoint`](./journal.md#the-entry), beside the value that displaced it — the field with no counterpart on a frame, and what makes a re-point undoable rather than merely observable. A `dissociate()` is kept the same way, its `previous` carrying the pointer in full, so a removal is as undoable as a re-point. No snapshot holds either: an association change bumps no `version`, so there is no moment at which one is taken.
...
**Version history is not a reference.** A `file-ref` field sitting in a snapshot does not hold bytes alive, for either the sweep or `deleteAttachment()`: counting it would make a file undeletable for the lifetime of any snapshot that ever mentioned it, which is exactly the intentional delete this model exists to keep possible. The consequence is that [`restoreVersion()`](./versioning.md#restore-semantics) can put back a `file-ref` naming bytes that have since been deleted — a **dangling file reference**, on the same terms and for the same reason a restore is the one write that can produce a [dangling parent](./data-model.md#reparenting): history is put back, not re-litigated against what is true now. Consumers handle one the way they handle any `fileId` that resolves to nothing, and `getAttachment()` answers `StackNotFoundError`.
