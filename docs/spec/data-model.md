...
It gets a verb rather than a slot in the `PATCH` body because the body of a content patch is the content: a `parentId` key there is a content field named `parentId`, which is a legal thing for a schema to declare, and no envelope can be added around it without also inviting a `createdAt` or an `entityId` that create-only rules would then have to refuse one at a time.
...
**A move may not make a record its own ancestor.** `create()` can never produce a cycle — a new record's ID names nothing yet — so `setParent()` is the single site where the hierarchy could stop being a tree, and it walks the proposed ancestor chain and refuses with `StackConflictError` (wire: **409**) when it arrives back at the record. Core itself never walks the hierarchy, but consumers do: a static-site generator derives a page's path from its `parentId` ancestors ([commons § `page`](../commons/page.md)), and any folder view recurses. A cycle would hang all of them.
