...
A millisecond therefore holds a bounded number of IDs, and how many is itself random: the suffix opens on a random draw and increments from there, so the room left runs anywhere from one ID to 32³. Spending it carries the next ID into the following millisecond — the prefix advances, a fresh suffix is drawn, and the order stays total. Minting never fails for want of room, so no caller meets a capacity it had no way to predict.

A carried ID's prefix therefore reads a millisecond later than the `createdAt` stamped beside it — the same relationship a backward clock step (NTP correction, suspend/resume) produces, where the generator's monotonic floor clamps the prefix forward so no ID sorts before ones already handed out while `createdAt` reports the wall clock honestly. Ordering is unaffected either way: queries sort by `createdAt` with `id` as the tiebreak, and both agree with creation order.
...
**A content patch merges at the top level only.** Each key the patch names is replaced whole: patching `{ address: { city: 'Lisbon' } }` sets `address` to exactly that object, dropping any `postcode` it held. This is not RFC 7396, which recurses — the merge is one level deep, and a caller meaning to change one field of a nested object sends that object as it should end up. Nesting is where a recursive merge stops being predictable (it cannot express "replace this object wholesale" at all), and one level is the depth at which "omitted means unchanged" stays something a caller can hold in their head.

**A key is read for presence, not for truthiness.** `unlisted: false` and `parentId: null` are changes and are applied; an omitted key is an aspect the write does not touch. A change set naming **no** key is refused with `StackBadRequestError` (wire: 400) — it addresses no aspect at all, so there is nothing for it to have failed to satisfy, and no reading of it a caller meant.

**A change that changes nothing writes nothing.** Every key present is compared against the Record as it stands, and a change set already satisfied in each one returns the Record unchanged: no new version, no snapshot, no event. The comparison is per key and the decision is over the whole set — a change set that moves one aspect and restates another still writes, and writes once. `associate()` and `dissociate()` answer the same way on their own.
...
**Authority is resolved per key, against the Record as it stands.** A caller who may edit content and a caller who may reshare are not the same caller, and naming both in one change set gives neither of them the other's reach — see [Access control § Composing a change set](./access-control.md#composing-a-change-set).
...
**A `parentId` a caller names must be a well-formed id naming a record that exists.** `mutate()` and `create()` check both, refusing a malformed id with `StackBadRequestError` (wire: **400**) and one naming no record with `StackConflictError` (wire: **409**). Format is checked first, so a bad id reports what is wrong with it rather than failing as a lookup that could never match; the same rule as a caller-supplied `id`, which is why `''` is not a `parentId` any more than it is a record id. Existence closes an asymmetry rather than adding a cost: [the reference gate](./access-control.md#reference-creation-gating) already refuses a non-owner a parent they cannot read, and a parent that isn't there is one nobody can read — only the owner's path skipped it.
...
The walk uses unscoped storage reads, so it sees links the requester cannot: a walk that skipped them would let a cycle be assembled through unreadable records and break the invariant for every reader. The requester learns only that some chain exists between two records they can already reach — read access to the destination having been required first — and integrity is worth that much more than the inference is worth closing.

**The walk is bounded at 64 levels, and the bound stops the walk rather than the move.** Past the cap it returns and the write proceeds. A chain that long is past what the check can speak to, not evidence of a loop, and refusing it would advertise an invariant core does not maintain — while blocking legitimate moves into a deep tree for good, since nothing shortens the chain. The bound is what keeps each move's cost fixed, and what guarantees the walk terminates on a chain that is already cyclic. **Depth itself is unbounded**: an ordinary `create()` under a parent never walks, and moving a subtree checks only the chain above it, so records can and do sit deeper than 64.

**The check is read-then-write, so it is advisory under concurrency**: two moves racing on opposite ends of one chain can each pass their own check and both land. Closing it properly means the invariant lives in the adapter, where the write is atomic — a graph constraint in the storage contract that every adapter would then implement, which is a decision about where acyclicity lives rather than a local fix. Same posture as [DID binding uniqueness](./identity.md#did-bindings).

So acyclicity is **a guardrail against the common accident — moving a container into its own descendant — and not a property to build on.** It is exact for a single writer within the cap, and nothing beyond that. **A consumer that walks `parentId` MUST carry a visited set or a depth bound of its own**; this is a requirement on consumers, not a suggestion. Core never walks the hierarchy, so a cycle costs core nothing — every read, filter and feed is single-level — but a static-site generator derives a page's path from its `parentId` ancestors ([commons § `page`](../commons/page.md)) and a folder view recurses, and a cycle hangs both. A cycle is always escapable: `mutate(id, { parentId: null })` reaches no container, so it is never refused, and moving any member to the root breaks the loop.

**A move is undone by a move.** No snapshot captures `parentId`, so [`restoreVersion()` never settles containment](./versioning.md#restore-semantics) — it puts content back and leaves the record where it sits. Undoing a move is another `mutate()` naming `parentId`, reading the origin off [the journal entry](./journal.md#the-inverse) the move appended, and it leaves the same trail every other write leaves: a new entry, not a rewritten one.
...
**`__proto__`, `constructor`, and `prototype` are rejected as top-level content keys** — on `create()` and in a content patch alike — with `StackValidationError` (422), independently of whether the Type declares them.
...
A content filter key is a **dot-separated path** (see [Filter](#filter)), so a field named `emails.value` and a path reaching `value` inside `emails` would be the same string asking two different questions. The ambiguity is removed from the field name rather than from the path: **a content field name may not contain `.`, `[`, `]`, `$`, `"`, `*`, or `#`** — rejected with `StackValidationError` (422) on `create()`, in a content patch, and on `commitMigration()`.
...
**`relatedTo` names a label, a target, or both — never neither**, and a filter naming neither is rejected with `StackBadRequestError` rather than widening to every Record carrying a relationship. Its target follows the same naming rules as a stored one (above), checked the same way. A `RelationshipTargetPattern` is the association's own target shape with the parts a query may leave open. Each half is a pattern: a bare `label` matches every target under it; an `external` target with no `id` matches the whole namespace, which is how a syndication tool asks what it has already published. A `record` target with no `stackUrl` matches only local targets — absence names this stack rather than acting as a wildcard, so a Record referenced in someone else's stack is reachable only by naming that stack. Matching is exact within a target kind and never across kinds.

**`attachment` is shaped like `relatedTo`: a label, a `fileId`, or both — never neither.** Both halves together match one association, so `{ label: 'cover', fileId: F }` finds a Record whose cover is `F`, not one that has some cover and some other reference to `F`. It asks about associations only. **`referencesFileId` asks the wider question** — does anything in this Record point at the file — and also counts a top-level `file-ref` content field. That is the question [`deleteAttachment()`'s reference check](./attachments.md#deleting-attachments) and [garbage collection](./attachments.md#garbage-collection) ask, and it cannot live inside `attachment`: a `file-ref` field has no label, so combined with one it would match under some filters and not others.

**"Carries any relationship at all" is deliberately not expressible.** `tags` and `attachment` have no match-any form either, so a relationship one would be the odd exception rather than a missing convenience — and a filter that can encode to nothing is a filter that can silently widen a query when it crosses the wire. The type refuses the empty filter rather than defining it, and `Stack.query()` refuses it again at runtime, where a filter decoded from query parameters is a plain object the type never saw.
...
A path that descends through a scalar, or through a field that isn't there, reaches no value and matches nothing — it is not an error. A path may be at most **32 segments**; longer is `StackBadRequestError` (400), as is an empty segment (`a..b`) or a segment containing a reserved character. Those are structural faults in the request, refused rather than answered with an empty result, on the same reasoning as a malformed cursor.
...
**A query result carries no count of the whole match.** `QueryResult` is `records` and `cursor`, and nothing else. The count of matching Records, unfiltered, would reveal how many Records exist that the requester cannot read — the cardinality the permission check just hid — so a [permission-scoped](./access-control.md) query could never report one, and neither could any response on the wire (see [Wire format § Response envelope](./wire-format.md#response-envelope)). That left a number reachable on exactly one path, a direct unscoped `Stack.query()` in process, which is the worst of both: an app written against it reads a number locally and `null` the moment the same code runs scoped or against a server. A field no caller can rely on is worse than no field, and computing it cost every caller a full count of the match on every page — including the scoped and wire paths, which then discarded it.

A caller that genuinely needs a count follows `cursor` to exhaustion and counts what it receives, which is the only number that was ever true for that requester anyway.
...
`content` filtering and `search` depend on adapter capabilities (see [Adapter capabilities](./adapters.md#adapter-capabilities)). `query()` fails loud rather than silently widening: `Stack.query()` checks `filter.content`/`filter.search` against `adapter.capabilities` before dispatching, and throws `StackBadRequestError` (`bad_request`/400) if the adapter hasn't declared the matching capability — local and remote behave identically.

**A refusal names the capability it was refused for.** `StackBadRequestError` carries a `capability` field holding the path into [adapter capabilities](./adapters.md#adapter-capabilities) the query needed — `'filter.content'`, `'filter.contentPresent'`, `'filter.search'`, `'sort.fields'` or `'sort.contentField'`. A client presenting the refusal in its own vocabulary reads the name from there rather than re-deriving it from the query, which is how one rule becomes two answers that can disagree; `adapter-api` raises `APIAdapterCapabilityError` carrying the same name. A `StackBadRequestError` thrown for the query's own shape — a malformed content path, a sort naming both a native field and a content field — carries none, because that is the caller's error at every capability level and naming a capability would point at a discovery entry that is not the problem. The field is local only: the [wire error body](./wire-format.md#error-responses) carries `bad_request` and a message, as it does for every other `StackBadRequestError`.

**`contentPresent` needs `filter.contentPresent` on top of a content reach**, a separate flag on the same reasoning the reach ladder rests on: a server promising to match a content value has not thereby promised to answer whether one is there, and reading it as such would hand a client the unfiltered superset that ignoring the filter produces. A multi-segment presence path needs the `'path'` rung too, exactly as a filter key does.
...
The repair claims no completeness against the engine's grammar, so adapters back it with a rule that does: **whatever reaches the engine as search text and fails to parse surfaces as `StackBadRequestError`** (`bad_request`/400), never as a raw engine error. Membership in the [error taxonomy](./wire-format.md#the-taxonomy-root) is what gives a server a status to answer with — an error outside it has no mapping, and ordinary search input would land as a 500. The rule is scoped to `filter.search` deliberately: every other clause is built from bound parameters, so a parse failure in one is the adapter's own bug and must not be relabelled as the caller's.

**Sanitization bounds a search's grammar, not its cost.** The FTS sanitizers strip the operators that make a query pathological to parse, but a syntactically ordinary search over a large index can still be expensive to execute. Full-text search therefore lands in the same best-effort-on-cost bucket as [a nested content path](#nested-content-paths), and a server owes it the same bound. See [Wire format § Bounding query cost](./wire-format.md#bounding-query-cost).
