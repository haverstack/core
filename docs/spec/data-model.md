...
**A `content` filter key names one top-level field, literally.** Undeclared content fields are permitted (above), so a key is an arbitrary string and never a path expression: `{ content: { 'a.b': 1 } }` asks for the top-level field called `a.b`, not `b` nested inside `a`, and `{ 'arr[0]': 1 }` asks for the field called `arr[0]`, not an array element. Nested content is [opaque to the query engine](#types) in v1 and there is no syntax for reaching into it. An adapter addressing content through a path language (a SQLite adapter's `json_extract`) MUST quote and escape the key into that language rather than interpolating it, so every key a write would accept is a key a filter can name and no key is silently reinterpreted as syntax; a key naming a field no record has is an ordinary zero-match filter, never an error.
