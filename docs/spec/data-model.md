...
**A key is read for presence, not for truthiness.** `unlisted: false` and `parentId: null` are changes and are applied; an omitted key is an aspect the write does not touch. A change set naming **no** key is refused with `StackQueryError` (wire: 400) — it addresses no aspect at all, so there is nothing for it to have failed to satisfy, and no reading of it a caller meant.
...
**`restoreVersion()` is exempt.** It is not a caller naming a destination — it is history being put back, and the container may have been hard-deleted since the snapshot was taken. Refusing there would let an unrelated deletion cost a record its content rollback, which is the recoverability the verb exists for. This is the same shape as validating snapshot content against the snapshot's own `typeId` rather than the record's current one: a restore honors history instead of re-litigating it. A restore can therefore still produce a dangling parent, and it is the only write that can. It is the only write that can dangle a [file reference](./attachments.md#garbage-collection) too, and for the same reason — a snapshot is put back as it stands, whatever has been deleted since.

**A record may not become its own ancestor.** Every site that adds a containment edge walks the proposed ancestor chain and refuses with `StackConflictError` (wire: **409**) on arriving back at the record: a `mutate()` naming `parentId`, a [`restoreVersion()`](./versioning.md#restore-semantics) whose snapshot names a different container, and a `create()` supplying **both** `id` and `parentId`. A generated ID names nothing, so an ordinary create cannot close a loop and skips the walk; a caller-supplied one can, since existing records may already point at it — though with the existence check above, a chain leading to a not-yet-created id can only have come from a restore or from an adapter write beneath `Stack`.
...
**A reparent rolls back like anything else.** A snapshot captures `parentId`, so `restoreVersion()` puts the record back in the container the snapshot was taken in — the root when the snapshot carries no `parentId`, since a snapshot spells containment the way a record does. Undoing a move needs no verb of its own and leaves the same trail every other undo leaves: a new version, not a rewritten one. See [Versioning § Restore semantics](./versioning.md#restore-semantics).
...
An attachment's optional `attachmentRecordId` is outside that identity: it names the `_attachment` record whose upload established the reference, so a reader can resolve the filename _this_ record was given rather than a byte-identical sibling's. `dissociate()` matches without it, and an `associate()` naming a different one — or naming none, which clears it — re-points the association the record already holds instead of adding a second reference to the same file. See [Attachments § Naming the upload a reference came from](./attachments.md#naming-the-upload-a-reference-came-from).
...
**A refusal names the capability it was refused for.** `StackQueryError` carries a `capability` field holding the path into [adapter capabilities](./adapters.md#adapter-capabilities) the query needed — `'filter.content'`, `'filter.contentPresent'`, `'filter.search'`, `'sort.fields'` or `'sort.contentField'`. A client presenting the refusal in its own vocabulary reads the name from there rather than re-deriving it from the query, which is how one rule becomes two answers that can disagree; `adapter-api` raises `APIAdapterCapabilityError` carrying the same name. A `StackQueryError` thrown for the query's own shape — a malformed content path, a sort naming both a native field and a content field — carries none, because that is the caller's error at every capability level and naming a capability would point at a discovery entry that is not the problem. The field is local only: the [wire error body](./wire-format.md#error-responses) carries `bad_request` and a message, as it does for every other `StackQueryError`.
