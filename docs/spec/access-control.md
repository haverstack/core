...
`Stack.grant()` is the owner-facing helper for creating grant records; `Stack.listGrants(target?)` and `Stack.revoke(target, grants)` are the read/undo counterparts. A `target` is a `GrantGrantee` — the same union a stored grant carries, so a target _is_ the grantee it writes:
...
// whole, role included.
await stack.revoke({ kind: 'entity', entityId: 'bob-entity-id' }, [
...
- **Every tier is spelled, and a target must name one.** `grant()`, `revoke()` and `listGrants()` reject a target naming no `kind`, an empty `entityId`, an empty `groupId`, or a group target without a `member`/`admin` role, with `StackQueryError`. The `_grant` schema requires `grantee`, so a Record arriving any other way — an unscoped `Stack`, a JSON import, a server mapping a request body, an adapter write — is refused at validation rather than stored. And evaluation reads the tier off `kind`: a grantee that is absent, names an unknown `kind`, or carries an empty `entityId` or `groupId` confers nothing however it came to exist. No field's loss widens a grant's reach, because reach is never spelled by absence. The `groupId` itself is not format-checked, and the Record it names must be in the `_group` family — both on the same terms as a record-level permission's `group` grantee (see [Record-level permissions](#record-level-permissions)).
