...
- **`parentId`, and a `relationship` whose target is a Record in this stack** (`{ scope: 'record' }` with no `stackUrl`), require read access to that Record.
