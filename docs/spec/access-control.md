...
- **Every tier is spelled, and a target must name one.** `grant()`, `revoke()` and `listGrants()` reject a target naming no `kind`, an empty `entityId`, an empty `groupId`, or a group target without a role, with `StackQueryError`. The `_grant` schema requires `grantee`, so a Record arriving any other way — an unscoped `Stack`, a JSON import, a server mapping a request body, an adapter write — is refused at validation rather than stored. A schema cannot say which fields each arm carries, since a closed `object` field holds one `properties` set, so **the arm's own fields are required on every write instead**: `create()`, `patchContent()`, `restoreVersion()` and `commitMigration()` refuse a `_grant` whose grantee names an unknown `kind`, or whose `entity` arm carries no `entityId`, or whose `group` arm carries no `groupId` or no `member`/`admin` role, with `StackValidationError`. Without that, an arm missing its own field would store, answer 200, and deny forever — a share that looks like it worked and never did. And evaluation reads the tier off `kind` a third time: a grantee that is absent, names an unknown `kind`, or carries an empty `entityId` or `groupId` confers nothing however it came to exist. No field's loss widens a grant's reach, because reach is never spelled by absence. The `groupId` itself is not format-checked, and the Record it names must be in the `_group` family — both on the same terms as a record-level permission's `group` grantee (see [Record-level permissions](#record-level-permissions)).
