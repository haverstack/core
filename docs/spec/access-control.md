...
| `setParent`                | Yes                         | Versioned — `restoreVersion()` puts the Record back in its prior container          |
...
`setPermissions()`, create-time `permissions` and `grant()` reject these with `StackValidationError`, and **evaluation refuses them again** — the same posture the ungrantable system families take. A `permissions` array or a `_grant` Record can also arrive from an unscoped `Stack`, a JSON import, or a server mapping a request body onto storage, and refusing only at the helper would hold only for what went through it. An entry or Grant carrying mutation without read confers nothing, however it came to exist.
...
- **Parenting into a container needs read on the container** — `parentId` is a reference, gated like any other, at creation and on a later `setParent()` alike (see [Reference-creation gating](#reference-creation-gating)). Share the container Record `read: true, write: false` (it holds no submissions; each is its own private Record), or use no parent and correlate through a content field or tag.
