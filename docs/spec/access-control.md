...
Two questions next to this one have their own documents: whether a Record is _enumerable_, which is separate from who may read it, is [Unlisted records](./unlisted.md); and what a requester learns when either layer refuses them is [Refusals and disclosure](./disclosure.md).

## Refused at the write, and again at evaluation

`grantType()`, `grantAccess()` and the change-set keys are not the only ways authority-bearing state reaches storage. An unscoped `Stack`, a JSON import, a server mapping a request body, an adapter write and a foreign server's response all produce permission elements and `_grant` Records that no helper vetted and no compiler saw.

So every rule below that decides reach is enforced twice: **refused at the write**, and **read as conferring nothing at evaluation**. A rule held only at the writing helper would hold only for what went through it, and what it was meant to refuse would instead store, answer 200, and deny forever — a share that looks like it worked and never did. Where a rule below says it is enforced "again at evaluation", this is what it means; the argument is not restated at each one.

Two consequences are general rather than per-rule:

- **Reach is never spelled by absence.** No dropped field widens anything — an absent grantee, an unknown `kind`, an empty `entityId` or `groupId` confers nothing rather than falling back to something wider.
- **An element or Record the evaluator does not recognize conveys nothing**, in either layer, whatever the rule at the write would have said about it.
...
- **A permission's grantee is its own shape**, not a [`RelationshipTarget`](./data-model.md#relationship-targets), which has no role and no group arm. It is the same `Grantee` a [type-level grant](#type-level-grants) names, so one value reaches either layer.
...
- **Two change-set keys**, each replacing only within its own domain. A `permission` or `anyone` kind named in the `associations` key — or a `tag`, `attachment` or `relationship` named in `permissions` — is a `StackBadRequestError`, not a validation error: the caller named the wrong surface rather than a malformed value. The refusal lives in `Stack`, so [every writer is held to it](#refused-at-the-write-and-again-at-evaluation).
...
**A key the gate read as inert is dropped, not forwarded.** The delta is computed against the Record the gate read; the write recomputes it against the Record as it then stands. Those are two reads, so a key left standing on the strength of the first would replace the ACL on the strength of the second — and the set a caller who cannot reshare supplied is, by then, one a concurrent grant has already moved past. Dropping it is not the silent narrowing [create-time `permissions`](#delegation-principal-and-subject) is refused to avoid: that key carries a request, and this one has just been shown to carry none. A caller who _can_ reshare keeps the ordinary replacing semantics, concurrency and all — which is why `grantAccess()`/`revokeAccess()` remain the spelling that survives two admins sharing one Record at once.
...
| Verb, via a `write` element                               | Reversible?                 | How                                                                                                                                                                                                                             |
| --------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| ----------------------------------------------------------- | -------------------------------------------------------------------- |
| A `write` element with no `read` reaching the same grantee  | The write bit is inert without one, `entity` and `group` alike       |
| Revoking a `read`, or an `anyone`, while its `write` stands | The same rule, reached from the side where a removal blinds a writer |
...
**An `anyone` element satisfies it for every grantee.** A world-readable Record leaves no writer blind, so a second `read` naming the writer would withhold nothing and refusing the set would be friction, not safety.

The post-state reading is what keeps that honest from the other direction, and the rule it enforces is the same one: **no write lands in a set whose grantee cannot read it**, whichever verb or key produced the set. So withdrawing the `anyone` while a `write` it was covering stands is refused — element-wise through `revokeAccess()`, or named in a `permissions` key that keeps the write — while the spellings that drop the write in the same breath are ordinary writes: `permissions: []` makes the Record private in one call, and a key naming the writer's own `read` in place of the `anyone` swaps one for the other in one call. Nothing here asks the owner to unwind in a particular order; only the set a call would produce is asked about.

`grantAccess()`/`revokeAccess()`, the `permissions` key at create time and after, and `grantType()` reject these with `StackValidationError`, and [evaluation refuses them again](#refused-at-the-write-and-again-at-evaluation): a `write` element with no `read` beside it, or a Grant carrying mutation without read, confers nothing however it came to exist.
...
**The read action must sit in the same `_grant` Record**, not merely somewhere in the grantee's set. A grant is revoked whole, at the granularity `grantType()` writes: a rule satisfied across two Records would let revoking the read one leave a mutate-without-read grant standing — arriving at exactly the configuration this rule refuses, without anyone having written it. Each grant is coherent on its own, so no revocation can produce an incoherent pair.
...
type GrantContent = {
  ...
  grantee: GrantGrantee; // Who it reaches — required
};
...
type GrantQuery = // What listTypeGrants() accepts
...
await stack.listTypeGrants(); // every grant record, any grantee
await stack.listTypeGrants({ kind: 'authenticated' }); // only default grants
await stack.listTypeGrants({ kind: 'entity', entityId: 'bob-entity-id' }); // grants naming Bob, grants naming a group he belongs to at a role he holds, plus every default grant — what currently applies to him
await stack.listTypeGrants({ kind: 'group', groupId: 'editors-group-id', role: 'member' }); // grants naming that exact group and role
await stack.listTypeGrants({ kind: 'group', groupId: 'editors-group-id', role: 'any' }); // every grant naming that group, whichever role

// The inverse of grantType(): soft-deletes the _grant record(s) matched by
// typeId baseId, action set and grantee — the same granularity grantType()
...
- **Every tier is spelled, and a target must name one.** `grantType()`, `revokeType()` and `listTypeGrants()` reject a target naming no `kind`, an empty `entityId`, an empty `groupId`, or a group target without a role, with `StackBadRequestError`. The `_grant` schema requires `grantee`, so a Record arriving [any other way](#refused-at-the-write-and-again-at-evaluation) is refused at validation rather than stored. A schema cannot say which fields each arm carries, since a closed `object` field holds one `properties` set, so **the arm's own fields are required on every write instead**: `create()`, `patchContent()`, `restoreVersion()` and `commitMigration()` refuse a `_grant` whose grantee names an unknown `kind`, or whose `entity` arm carries no `entityId`, or whose `group` arm carries no `groupId` or no `member`/`admin` role, with `StackValidationError`. And evaluation reads the tier off `kind` again: a grantee that is absent, names an unknown `kind`, or carries an empty `entityId` or `groupId` confers nothing.
...
**Deletion is the only lifecycle state that withdraws a grant.** A `_grant` Record is read with [unlisted Records included](./unlisted.md), so withholding one from enumeration changes nothing about what it confers: listing state decides which queries enumerate a Record, never what it means, and a grant silently disarmed by an `unlisted` key would be a revocation spelled by a flag that is defined not to revoke. The same read serves `listTypeGrants()` and `revokeType()`, so an unlisted grant stays visible to the owner who wants to withdraw it — a grant that still confers and cannot be found is the one state worth refusing outright.

- **`listTypeGrants()` answers identity, except for the `entity` arm.** `listTypeGrants(X)` returns the grants `grantType(X, …)` would have written with the same argument — a group query matches the stored grantee whole, `role` included, so the listing and `revokeType()` never disagree about what an argument names. The `entity` arm is the exception and answers **coverage**: the grants that currently _apply_ to that entity, which includes grants naming a group they belong to and every default grant. That result is therefore **not a preview of what `revokeType()` would withdraw** — feeding it back to `revokeType()` would aim at grants that were never written for that entity. Coverage is well-formed only for a real principal; a group and a role describe a grant, not someone who could hold one.
- **A group listing widens affirmatively, with `role: 'any'`.** `listTypeGrants({ kind: 'group', groupId, role: 'any' })` returns every grant naming that group, whichever role it carries — the one question exact matching cannot ask. `'any'` is listing-only: it is not a role an entity can hold, `grantType()` and `revokeType()` reject it, and it can never reach storage, so a query can say it and a grantee cannot. Spelling the widening this way rather than by omitting `role` keeps the rule above intact — breadth is never spelled by absence, in a query any more than in a grantee.
- **`revokeType()` returns what it withdrew, and matching nothing is not an error.** The result is the grants as they stood, so a caller can say what was taken away, and an empty array says plainly that nothing matched. It stays a no-op rather than a failure because `revokeType()` matches on an exact `actions` set and the whole target: a grant already withdrawn, or never written, is the ordinary case, and re-running a revocation must be safe. This mirrors `revokeAccess()`, where an element the Record does not carry is a no-op returning the Record as it stands. The shapes differ because the targets do: `revokeAccess()` edits one Record, while one `revokeType()` target can match several `_grant` Records — `grantType()` does not deduplicate, and grants written against the baseId and against a versioned TypeId of the same family match alike.
...
| Purge                                                         | Irreversible; the subject holds soft delete already                                                                  |
...
`appId` and `permissions` are deliberately **not** gated by this: `appId` is self-reported, untrusted metadata everywhere (see [App](./identity.md#app) for what can and cannot be checked after the fact), never a permission input. `permissions` at create time is consistent with the same key's owner-or-creator policy after it — a contributor authoring a Record in your Stack can already widen its access up to and including world-readable; create-time is the same capability exercised earlier, not a new one. That holds for every requester who genuinely holds the reshare capability, which is every undelegated one, and [not for a delegated principal](#delegation-principal-and-subject), where the key is refused outright rather than ungated.
...
`ScopedStack`'s group-membership check only resolves `_group` Records living in the same stack as the Record being accessed — it does not yet implement the cross-stack case described above. A server relying on cross-stack groups must still handle that case itself. The same limitation applies to a type-level grant's `group` grantee (above): a group-targeted grant only resolves a roster in the same stack as the `_grant` Record.
