...
`grantAccess()`/`revokeAccess()`, the `permissions` key at create time and after, and `grant()` reject these with `StackValidationError`, and [evaluation refuses them again](#refused-at-the-write-and-again-at-evaluation): a `write` element with no `read` beside it, or a Grant carrying mutation without read, confers nothing however it came to exist.
...
await stack.listGrants({ kind: 'entity', entityId: 'bob-entity-id' }); // grants naming Bob, grants naming a group he belongs to at a role he holds, plus every default grant — what currently applies to him
...
- **Some system types can't be granted at all**: `grant()` refuses `_grant`, `_config`, and `_app`. Each would hand the grantee the machinery the model rests on — minting their own grants, rewriting stack ownership, or registering an app card claiming a DID that isn't theirs (see [App](./identity.md#app)). Other reserved types (`_attachment`, `_entity`, `_group`) stay grantable. The refusal is enforced [again at evaluation](#refused-at-the-write-and-again-at-evaluation): a `_grant` Record naming one of these families confers nothing, however it came to exist.
...
- **Every tier is spelled, and a target must name one.** `grant()`, `revoke()` and `listGrants()` reject a target naming no `kind`, an empty `entityId`, an empty `groupId`, or a group target without a role, with `StackBadRequestError`. The `_grant` schema requires `grantee`, so a Record arriving [any other way](#refused-at-the-write-and-again-at-evaluation) is refused at validation rather than stored. A schema cannot say which fields each arm carries, since a closed `object` field holds one `properties` set, so **the arm's own fields are required on every write instead**: `create()`, `patchContent()`, `restoreVersion()` and `commitMigration()` refuse a `_grant` whose grantee names an unknown `kind`, or whose `entity` arm carries no `entityId`, or whose `group` arm carries no `groupId` or no `member`/`admin` role, with `StackValidationError`. And evaluation reads the tier off `kind` again: a grantee that is absent, names an unknown `kind`, or carries an empty `entityId` or `groupId` confers nothing.
...
**Deletion is the only lifecycle state that withdraws a grant.** A `_grant` Record is read with [unlisted Records included](./unlisted.md), so withholding one from enumeration changes nothing about what it confers: listing state decides which queries enumerate a Record, never what it means, and a grant silently disarmed by an `unlisted` key would be a revocation spelled by a flag that is defined not to revoke. The same read serves `listGrants()` and `revoke()`, so an unlisted grant stays visible to the owner who wants to withdraw it — a grant that still confers and cannot be found is the one state worth refusing outright.

- **`listGrants()` answers identity, except for the `entity` arm.** `listGrants(X)` returns the grants `grant(X, …)` would have written with the same argument — a group query matches the stored grantee whole, `role` included, so the listing and `revoke()` never disagree about what an argument names. The `entity` arm is the exception and answers **coverage**: the grants that currently _apply_ to that entity, which includes grants naming a group they belong to and every default grant. That result is therefore **not a preview of what `revoke()` would withdraw** — feeding it back to `revoke()` would aim at grants that were never written for that entity. Coverage is well-formed only for a real principal; a group and a role describe a grant, not someone who could hold one.
- **A group listing widens affirmatively, with `role: 'any'`.** `listGrants({ kind: 'group', groupId, role: 'any' })` returns every grant naming that group, whichever role it carries — the one question exact matching cannot ask. `'any'` is listing-only: it is not a role an entity can hold, `grant()` and `revoke()` reject it, and it can never reach storage, so a query can say it and a grantee cannot. Spelling the widening this way rather than by omitting `role` keeps the rule above intact — breadth is never spelled by absence, in a query any more than in a grantee.
- **`revoke()` returns what it withdrew, and matching nothing is not an error.** The result is the grants as they stood, so a caller can say what was taken away, and an empty array says plainly that nothing matched. It stays a no-op rather than a failure because `revoke()` matches on an exact `actions` set and the whole target: a grant already withdrawn, or never written, is the ordinary case, and re-running a revocation must be safe. This mirrors `revokeAccess()`, where an element the Record does not carry is a no-op returning the Record as it stands.
