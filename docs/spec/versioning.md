...
Version history is managed by the library as a side channel — apps do not manage it directly. It is one of the two durable tiers a mutation writes; the other is [the change journal](./journal.md), which records what each change moved rather than what a rollback could put back.

## Version history

**Two tiers.** Every mutation of a Record that touches content or permissions — a [change set](./data-model.md#mutations) naming `contentPatch` and/or `permissions`, a soft delete, an undelete, a migration commit, a restore — snapshots the Record's prior full state and bumps `version` exactly once. **One call is one version**, however many of those aspects it moved: a change set that edits content and replaces the ACL produces a single snapshot of how it stood before either. A mutation that changes nothing (restating a deep-equal permission set, deleting an already-deleted Record) is a no-op: no bump, no snapshot. Hard delete is the one exception among these — it destroys the Record and its version history outright, so there's nothing to snapshot.
...
`associate()`/`dissociate()` — and a change set naming only `associations`, `parentId` and/or `unlisted` — are the second tier, and never bump `version`, touch `updatedAt`, or snapshot. Each of those three aspects is a write whose prior state [the change journal](./journal.md) keeps in full, so a snapshot would preserve nothing a reader cannot already reach: a move's origin is the entry's `previousParentId`, a listing transition's inverse is the `unlist`/`list` op's own opposite, and an association delta is the entry's `associations`. That is the whole reason version history exists — recoverability for a write that can't otherwise be undone — and none of the three needs it. Filing them here anyway would cost a full duplicate copy of `content` apiece for state the entry already holds, and in `unlisted`'s case would bump `version` while storing nothing about the aspect that caused the bump. Associations carry one minor exception: re-pointing or clearing an attachment's `attachmentRecordId` overwrites a value in place, and the change feed's `associationsAdded` reports only the new one (see [Events § The event shape](./events.md#the-event-shape)) — the value it replaced is kept by the journal, which is where an association's prior state is recovered from generally, so the exception is the feed's rather than the model's. A change set that names any of the three alongside a version-bumping aspect still produces exactly one version covering everything it moved; they just aren't among the aspects that decide whether the call bumps at all. The same completeness is why a restore leaves all three where they stand and why `associate()`/`dissociate()` take no `ifVersion` — see [Restore semantics](#restore-semantics) and [Optimistic concurrency](#optimistic-concurrency-ifversion).
...
**A snapshot carries only what a snapshot preserves.** `content` and the `typeId` it is read under, the actors, and `permissions` for audit. There is no `parentId` and no `unlistedAt`: neither aspect bumps `version`, so no version is ever taken of either, and a server that emits one is writing a key every client drops.
...
- `stack.restoreVersion(recordId, version, opts?)` — revert to a prior version. Restores `content` and `typeId`, and **restores nothing else**. `permissions` is owner/creator territory (see [Access control](./access-control.md#the-write-bit-a-recoverability-trust-model)), and silently reverting an ACL as a side effect of a content rollback would be a surprise nobody wants — it's captured in the snapshot for audit and deliberate owner action, not automatic restore. `parentId`, `unlistedAt` and `associations` aren't in the snapshot at all to restore from — see [Restore semantics](#restore-semantics). The snapshot deliberately does not capture `appId`, so restore never reverts an app reattribution — that field keeps its current value.

**Why `permissions` is captured but never restored.** It is the one aspect the snapshot holds that a rollback does not put back, because the two jobs pull apart: permissions decide who may reach the Record, so reverting them silently would widen or narrow access as a side effect of a verb the caller asked for its content, while an owner auditing a sharing history needs to see what the ACL was. Capturing without restoring serves both. `appId` is not policy either way: it names the software that authored the Record, a create-time fact that no later write moves, so there is nothing for a rollback to revert it to.
...
Independently, snapshot `permissions` — audit data by design — are **stripped from every `RecordVersion` returned to anyone but the owner acting as itself**, including a write-holder who passes the history gate, and including a delegated request whose principal is the owner. A write-holder never needs the ACL trail to undo content, and a snapshot's `permissions` are the stack's sharing graph, which [delegation](./access-control.md#delegation-principal-and-subject) is not a route to. `entityId` (the Record's author, per [above](#version-history)) is not stripped — it is the same value the live Record already exposes to every reader, and far less sensitive than a permissions history. `permissions` is the only field with anything to strip: everything else a snapshot carries is content, its type, or an actor the live Record already names.
...
It is also what lets [the invariant](./identity.md#group) hold without a check here. A restore cannot move the roster, so it cannot be the write that empties one, and no version of a `_group` is ever unrestorable on that ground. The rule needs no adapter-side flag either: `StackRecordAdapter.restoreVersion()` never receives an association list to write, so there is nothing for a restore and the Record to disagree about.

**`restoreVersion()` re-runs reference-creation checks against the snapshot, for non-owner requesters.** Restoring re-attaches whatever file-ref content fields the target snapshot carried. For a write-holder who is not the owner, that could re-convey access to a file they can no longer reach today — the reference was legitimate when created, but access has since moved on. `ScopedStack.restoreVersion()` closes this by applying the same [reference-creation gating](./access-control.md#reference-creation-gating) `associate()`/`create()` apply, against the snapshot's file-ref fields: a file-ref field the requester couldn't currently attach fresh is rejected (`StackPermissionError`). There is no containment or `associations` case to gate: a snapshot carries neither, so a restore introduces neither. The owner acting as itself is exempt, per the same recoverability principle that exempts them from every other write-path gate — but an owner principal acting for someone else is not, since the gate resolves against the subject and it is the subject's reach a restore would widen.
...
`version` is not conflict detection by itself — it exists to power rollback and soft-delete recovery. Nothing reads or writes it unless a caller opts in. Every version-bumping mutating method (`mutate`, `patchContent`, `delete`, `undelete`, `restoreVersion`, `commitMigration`) accepts an optional `ifVersion`:
...
**It does not reach attachment bytes, and erasing those is a second step.** A `fileId` is a content hash: one file backs any number of Records, so byte lifetime cannot depend on which Record happened to be purged — that is the coupling [garbage collection](./attachments.md#garbage-collection) already rejects, and attachment deletion stays intentional. But the purge destroys the only rows naming those files, so `delete()` reports them and the caller finishes the job:
...
`referencedFileIds` names the Record's attachment associations and `file-ref` content fields, deduplicated, and is empty on a soft delete, which strands nothing. It is **information, not action**: nothing is deleted by being named, and `deleteAttachment()` still refuses a file another Record references. A purge for harmful content that skips the second step leaves the bytes reachable to whoever can already name the hash. See [Attachments § A purge strands the bytes it referenced](./attachments.md#a-purge-strands-the-bytes-it-referenced).
