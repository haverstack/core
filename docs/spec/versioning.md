...
**A restore rolls back a Group's membership, not its administration.** On a `_group` Record, `member` associations restore from the snapshot like any other association, but `admin` entries carry forward from the Record as it currently stands — the snapshot's are not put back, and the current ones are not taken away. This is the stance restore already takes on `permissions`, applied to the roster entries that are authority rather than data: silently reshuffling who administers a Group as a side effect of rolling back its display name is the same surprise, and rolling _forward_ is the worse half of it — a restore of old content would otherwise re-grant management to an `admin` who had been deliberately removed.

It is also what lets [the invariant](./identity.md#group) hold without making any version unrestorable. A snapshot taken when the roster had no `admin` — or one whose admins have all since been removed — restores like any other, because the admins it would have to produce are the ones the Record already has. No restore is ever refused on this ground, and none can leave a Group management-orphaned. Recovering a _former_ admin set is then a deliberate `associate()`, which is the point.
