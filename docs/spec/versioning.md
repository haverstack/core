...
**Every requester who passes that gate sees the same rows.** A snapshot carries content, its type, and an actor the live Record already names — nothing a requester holding the mutate surface is denied — so there is no per-requester projection of a `RecordVersion`. A Record's sharing history is the [journal's](./journal.md) to tell, and the journal is gated the same way.
