...
Everything else a Record carries — containment, listing, `associations` and `permissions` alike — has a delta the [journal](./journal.md) keeps in full, so a snapshot would preserve nothing a reader could not otherwise reach. Those writes bump nothing.
...
**A snapshot carries only what a snapshot preserves.** `content`, the `typeId` it is read under, and the actors. There is no `parentId`, no `unlistedAt`, no `associations` and no `permissions`: none of those aspects bumps `version`, so no version is ever taken of one, and a server that emits any of them is writing a key every client drops.
...
**Every requester who passes that gate sees the same `RecordVersion` rows.** A snapshot carries content, its type, and an actor the live Record already names — nothing a requester holding the mutate surface is denied — so there is no per-requester projection of a version.

A Record's sharing history is the [journal's](./journal.md#reading-it) to tell, and it is the one thing history does project per requester: the journal names _that_ a permission set moved to everyone who passes this gate, and who it moved to only to a requester who may reshare the Record. A write-holder never needs the ACL trail to undo content, and a Record's grantees — current and past — are not something [delegation](./access-control.md#delegation-principal-and-subject) is a route to.
