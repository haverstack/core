...
- **Inputs** spell the root `null` — `setParent()`'s argument, the `PUT /records/:id/parent` body, a `RecordFilter`, the `?parentId=null` query string. An input has to tell "the root" apart from "not specified", so it needs the extra spelling.
