...
type RecordVersion = {
  ...
  // No `associations` field: associate()/dissociate() never bump, so no
  // version ever snapshots the association set.
};
```
...
**A restore never rolls back any Record's associations, `_group` or otherwise.** `content` and `parentId` roll back as they do anywhere; a Record's `associations` are left exactly as they stand — there is no snapshot's list to put back, since [snapshots don't capture associations](#version-history) in the first place, and the current ones are not taken away either. This used to be a `_group`-only carve-out, on the grounds that a roster is authority rather than data: a Group's `member` and `admin` entries are what [group ACLs](./access-control.md#record-level-permissions) and [group-targeted grants](./access-control.md#type-level-grants) resolve against, so rolling one back would silently re-grant access — management to an `admin` who had been deliberately removed, reach to a `member` who had been dropped — as a side effect of a verb the caller asked for its content. That reasoning turned out to generalize: associations don't need restore-based recovery the way content does (see [Version history](#version-history)), so the carve-out is now the rule for every Record, and there is no more `_group`-specific exception to state. Recovering a former roster — or any other association set — is a deliberate `associate()`, which is the point.

It is also what lets [the invariant](./identity.md#group) hold without a check here. A restore cannot move the roster, so it cannot be the write that empties one, and no version of a `_group` is ever unrestorable on that ground. The rule needs no adapter-side flag any more either: `StackRecordAdapter.restoreVersion()` never receives an association list to write in the first place, so there is nothing a restore could disagree with what the Record already holds.
...
**`associate()`/`dissociate()` take no `ifVersion`.** They never bump `version`, so there is nothing for a precondition on it to guard, and they can't offer one that would silently do nothing. This isn't a gap: concurrent `associate`/`dissociate` calls are set add/remove operations that compose correctly regardless of order, unlike arbitrary content patches — the strong OCC guarantee content needs isn't needed here. A change set that mixes `associations` with a version-bumping aspect is unaffected — `mutate()`'s `ifVersion` still guards the whole call, the same as any other change set.
