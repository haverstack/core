...
It is also what lets [the at-least-one-admin invariant](./identity.md#group) hold without a check here: `StackRecordAdapter.restoreVersion()` never receives an association list to write, so no version of a `_group` is ever unrestorable on that ground.
