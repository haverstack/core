...
**SSE is the only transport in this version.** The reasoning is recorded so the decision can be revisited on a fact rather than re-argued.

What it would be: `GET /changes?since=<seq>&wait=<seconds>` with `Accept: application/json`, returning `{ frames: [...], seq, reset? }` — the same frames, the same cursor, the same `reset` semantics, held open until an event arrives or `wait` elapses. The frame vocabulary is transport-independent by construction, so this is a delivery mechanism rather than a second design.

**For:** some runtimes cannot stream a response at all, and a server on one can serve long-poll and cannot serve SSE — this is the whole case. Buffering proxies and CDNs also break SSE in ways that are hard to diagnose from the client, while a poll is an ordinary request that eventually returns. And it closes the stream-outlives-its-authority hazard for free: each poll is a fresh authenticated request, so a revoked token stops delivery at the next one without the lifetime bound the checklist above demands.

**Against:** it does not lower the implementation floor, which is the thing it looks like it does. A server cannot serve this feed by polling its own storage — `op` and `kind` are knowable only at mutation time, a purge leaves nothing behind to find, and `updatedAt` ordering cannot separate an `associate` from an `update`. So the server still captures events at mutation time and still buffers them per cursor; long-poll changes only how the buffer is drained, and a server reaching for it expecting "just a query with a cursor" ships a feed that silently loses hard deletes. It also obliges every client to implement both, which reproduces the failure the handshake was added to prevent: "the same client works against `localhost` or a remote provider" becomes false at connect. Every frame fixture would need a second encoding. And if both transports are normative and one is easier, some servers ship only the easier one and clients inherit seconds-scale latency from a design that had none — while mitigating that by making SSE a MUST excludes the very runtime long-poll was for.

Deferring costs nothing structurally: `transports` is an array, and adding an entry is additive under [version negotiation](#version-negotiation). **The trigger to revisit is what a reference server runs on.** A long-lived process — a container, a VM, a self-hosted box, which is the deployment a personal stack usually gets — makes SSE alone right, and long-poll a later additive answer to a real user on a constrained platform. A function runtime that cannot hold a response open makes long-poll a v1 requirement, because otherwise the reference server cannot implement its own spec.
