...
`@haverstack/core/wire` exports a conforming implementation of the whole body — `createOptionsFromWireRecord(body, session, ownerEntityId)`, returning the arguments `ScopedStack.create()` takes, with `createdBy` absent from the return type rather than merely unread. `isOwnerActingAlone()` is exported beside it, since a server owes the same determination on hard delete, `commitMigration()` and `includeUnlisted`. The rules above stay normative for a server written in another language.
