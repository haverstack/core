...
The record adapters sanitize a `search` string's _complexity_ — wildcards stripped, `NEAR` neutralized, parenthesis nesting capped — but not its _cost_. A syntactically modest query over a large index can still be expensive, and both SQLite engines run synchronously in-process: sql.js is WASM that blocks the JavaScript event loop, and native `node:sqlite` blocks the calling thread. There is no timeout to set from inside the call, because there is no other thread left to fire it. On a personal Stack this bounds nothing worse than the requester's own session; on a server, one expensive search stalls every other request sharing that thread.
