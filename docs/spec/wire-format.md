...
Two more sit in code this spec doesn't reach but a server does: `entityId` and `principalId` [must be ignored on input](#records), and a session's two identities must reach `ScopedStack` in the right order — `Stack.forSession()` takes the pair whole for that reason. A server built on core reaches the first through `createOptionsFromWireRecord()`, which cannot return either field.
