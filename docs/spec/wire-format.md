...
`@haverstack/core/wire` exports a conforming implementation of the whole body: `createOptionsFromWireRecord(body, session, ownerEntityId)` returns the `typeId`, `content` and options `ScopedStack.create()` takes, with each field's disposition structural. `entityId` and `principalId` are absent from the returned type rather than merely unread; `createdAt`/`updatedAt` are dropped by value for anyone but the owner acting alone; `unlistedAt` becomes `unlisted: true` for everyone, so the refusal stays `ScopedStack`'s. `isOwnerActingAlone(session, ownerEntityId)` is exported beside it, since the same tier gates hard delete, `commitMigration()` and `includeUnlisted`, and computing it as "is the owner" is a privilege bug with no symptom. The rules above stay normative for a server written in another language.

**Which of the two error classes a malformed body earns follows one question: does the failure name a field of the record being written?** A body that is not a create request at all — no `typeId`, no `content` — is `StackQueryError` (400), the same as any other unparseable request. A body that is a create request but carries a field the record cannot take — an unparseable `createdAt`, a non-string `id`, a `permissions` that is not an array — is `StackValidationError` (422), which carries the field path that distinction exists to report. This is the split `Stack.create()` already applies when it reports an `id` that disagrees with `createdAt` as a validation error against `path: "id"`. A present field is never silently dropped either way: a dropped `id` mints a different record than the one the client asked for, and a dropped `unlistedAt` publishes one the client meant to withhold. A `null` is refused rather than read as absent: a record body's optional fields are present or absent, and this protocol spends that spelling on "remove this field" in a `PATCH` body.
