...
- **Create** stamps the creator as the first `admin` (a `relationship` association added at create time), so the invariant is true from the Record's first version.
