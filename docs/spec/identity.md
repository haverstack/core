...
**Deferred alongside it: proving a server↔owner binding.** A server states which owner's stack it hosts and signs nothing, because it holds none of the owner's keys ([Wire format § Identity is trusted on transport](./wire-format.md#identity-is-trusted-on-transport)). Closing that needs an identity a server can speak for without holding the owner's key — a delegation the owner signs, or a stack-hosted method whose document names its own host — which is the same problem this section defers, approached from the other side. Until then a client's recourse is `expectedOwner`: state the DID you expect, and refuse a server that reports a different one.
