...
Vitest, in each package's `tests/`. `@haverstack/core/testing` provides `MemoryAdapter` (declares `filter.content: 'path'`, as every local adapter must) and `IncapableMemoryAdapter` (declares `'none'`, for exercising capability-gated fallbacks).
